...
namespace Illuminate\Hashing\Tests;
...
class HasherTest extends CIUnitTestCase
{
    ...
    public function testWithLoadFromStaticMethod()
    {
        ...
        $this->assertNotSame('password', $value);
        $this->assertTrue(Hash::check('password', $value));
        $this->assertFalse(Hash::needsRehash($value));
        $this->assertTrue(Hash::needsRehash($value, ['rounds' => 1]));
        $this->assertSame('bcrypt', password_get_info($value)['algoName']);
    }
}